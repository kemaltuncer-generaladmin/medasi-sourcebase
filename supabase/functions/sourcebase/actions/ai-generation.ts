...
async function extractTextFromDriveFile(userId: string, fileId: string) {
  ...
  const file = files[0];
  const metadataExtraction = extractionResultFromMetadata(file);
  if (metadataExtraction) return metadataExtraction;

  const bucket = storageBucket(file);
  const objectName = storageObjectName(file);
...
function extractionResultFromMetadata(file: Record<string, unknown>) {
  ...
  const text = sanitizeSourceText(
    extraction.text?.toString() ??
      metadata.sourceText?.toString() ??
      "",
  );
  if (!text.trim()) return null;

  const pageCount = numericOption(
    file.page_count ?? extraction.pageCount ?? extraction.page_count,
  );
  return {
    text,
    pageCount,
    ...
    metadata: {
      fileType: String(file.file_type ?? extraction.fileType ?? "client"),
      extractedAt: extraction.extractedAt?.toString() ??
        new Date().toISOString(),
      charCount: numericOption(extraction.charCount) ?? text.length,
      wordCount: numericOption(extraction.wordCount) ??
        text.split(/\s+/).filter((word) => word.length > 0).length,
    },
  };
}

function storageBucket(row: Record<string, unknown>) {
  return String(row.storage_bucket ?? row[legacyStorageKey("bucket")] ?? "");
}

function storageObjectName(row: Record<string, unknown>) {
  return String(
    row.storage_object_name ?? row[legacyStorageKey("object_name")] ?? "",
  );
}

function legacyStorageKey(suffix: "bucket" | "object_name") {
