...
struct SourceBaseGenerationContract {
    ...
    private static func aiBrief(for kind: GeneratedKind, mode: String, sourceLabel: String) -> String {
        ...
        Sağlık bilimleri öğrencisinin ihtiyacı açık (öğrencinin bölümüne göre özelleş — insan hekimliği varsayma): aktif hatırlama, spaced review, interleaving, görsel/işitsel çift kodlama, vaka/uygulama üzerinden klinik akıl yürütme, kaynak açığı ve kırmızı bayrak görünürlüğü. Çıktı öğrenciyi cevabı görmeden önce düşünmeye zorlamalı; çözümü doğrudan verip akıl yürütme adımını atlamamalı. \
